...
// ============================================
// 2. CV BUTTON FUNCTIONALITY & PDF CHECK
// ============================================

function initializeCVButton() {
    const cvButton = document.querySelector('.btn-cv');
    
    if (cvButton) {
        // Check if PDF exists before allowing download
        cvButton.addEventListener('click', function(e) {
            const pdfUrl = this.getAttribute('href');
            
            // Show downloading state
            const originalContent = this.innerHTML;
            this.innerHTML = '<svg class="btn-cv-icon" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> Preparing...';
            this.disabled = true;
            
            // Simulate download with fetch to check PDF availability
            fetch(pdfUrl, { method: 'HEAD' })
                .then(response => {
                    if (response.ok) {
                        // PDF exists, allow download
                        setTimeout(() => {
                ...
                .catch(error => {
                    // Network error or CORS issue - still allow download
...
function showCVFallbackMessage() {
    showNotification('CV file is currently being updated. Please try again shortly or contact directly.', 'info');
}

function createRippleEffect(event) {
    const btn = event.target.closest('.btn-cv');
    if (!btn) return;
...
window.addEventListener('load', function() {
    ...
    // Initialize CV button functionality
    initializeCVButton();
});

// ============================================
// 13. PRINT STYLESHEET SUPPORT
// ============================================
...
function enhanceAccessibility() {
    ...
    const cvButton = document.querySelector('.btn-cv');
    if (cvButton && !cvButton.getAttribute('aria-label')) {
        cvButton.setAttribute('aria-label', 'Download CV in PDF format');
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize CV button on DOM ready
    initializeCVButton();
});
